...
/*   Updated: 2023/07/27 10:21:06 by ynafiss          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub.h"

void	move_right(t_mlx *p, char **map)
{
	if (map[(int)((p->y_p - sin(p->angle - M_PI / 2) * 20) / 50)] \
	[(int)((p->x_p + cos(p->angle - M_PI / 2) * 20) / 50)] != '1')
	{
...
void	ft_move_dir2(char **map, t_mlx *p)
{
	if (p->left == 1)
	{
		...
		[(int)((p->x_p + cos(p->angle + M_PI / 2) * 20) / 50)] != '1')
		{		
			if (map[(int)((p->y_p) / 50)][(int) \
			((p->x_p + cos(p->angle + M_PI / 2) * 20) / 50)] != '1')
				p->x_p = p->x_p + cos(p->angle + M_PI / 2) * 4;
			if (map[(int)((p->y_p - sin(p->angle + M_PI / 2) * 20)
					/ 50)][(int)(p->x_p / 50)] != '1')
				p->y_p = p->y_p - sin(p->angle + M_PI / 2) * 4;
...
void	move_dwon(t_mlx *p, char **map)
{
	if (map[(int)((p->y_p + sin(p->angle) * 20) / 50)] \
	[(int)((p->x_p - cos(p->angle) * 20) / 50)] != '1')
	{
